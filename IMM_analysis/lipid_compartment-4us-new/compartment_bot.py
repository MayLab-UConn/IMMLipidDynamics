...
print('masksum...........................................')
print(digitized_mask.sum(axis=1))
total_popc =pd.DataFrame(popc_bin/10463)
frac_popc =pd.DataFrame(PC_fract)
print (total_popc.sum(axis=1))
print (frac_popc.sum(axis=1))
#########################################################################################
print (total_popc)
print ('is it two columns')
print (total_popc.iloc[:, 0:2])
print ('frac popc two coloumns...............................................')
print (frac_popc.iloc[:, 0:2])
...
frac_popc_cyl_def = pd.DataFrame(((frac_popc.iloc[:, 0:10].mean(axis=1))))
print ('checking new cylinder regions')
print (frac_popc_cyl_def)
bot_popc_cyl_frac = ((frac_popc_cyl_def-frac_popc_cyl_def.iloc[0,:]))
frac_popc_junc_def = pd.DataFrame(((frac_popc.iloc[:, 11:24].mean(axis=1))))
bot_popc_junc_frac = ((frac_popc_junc_def-frac_popc_junc_def.iloc[0,:]))
frac_popc_flat_def = pd.DataFrame(((frac_popc.iloc[:, 24:].mean(axis=1))))
bot_popc_flat_frac = ((frac_popc_flat_def-frac_popc_flat_def.iloc[0,:]))

np.savetxt('bot_popc_cyl.txt',bot_popc_cyl_frac)
np.savetxt('bot_popc_junc.txt',bot_popc_junc_frac)
np.savetxt('bot_popc_flat.txt',bot_popc_flat_frac)
np.savetxt('bot_popc_cyl_def.txt',frac_popc_cyl_def)
np.savetxt('bot_popc_junc_def.txt',frac_popc_junc_def)
np.savetxt('bot_popc_flat_def.txt',frac_popc_flat_def)
print (PC_fract)

PE_fract = np.zeros((n_frames, n_bins))
test_bin = np.zeros((n_frames, n_bins))
for i in range(digitized_coords.min(), digitized_coords.max() + 1):
    digitized_mask = (digitized_coords == i)
    PE_fract[:, i - 1] = digitized_mask[:, TEST].sum(axis=1) / digitized_mask.sum(axis=1)
    test_bin[:, i-1]=(digitized_mask[:, TEST].sum(axis=1))
np.savetxt('test_bin.txt', test_bin)
total_test =pd.DataFrame(test_bin/2526)
frac_test =pd.DataFrame(PE_fract)
...
frac_test_cyl_def = pd.DataFrame(((frac_test.iloc[:, 0:10].mean(axis=1))))
print ('checking new cylinder regions-test')
bot_test_cyl_frac = ((frac_test_cyl_def-frac_test_cyl_def.iloc[0,:]))
frac_test_junc_def = pd.DataFrame(((frac_test.iloc[:, 11:24].mean(axis=1))))
bot_test_junc_frac = ((frac_test_junc_def-frac_test_junc_def.iloc[0,:]))
frac_test_flat_def = pd.DataFrame(((frac_test.iloc[:, 24:].mean(axis=1))))
bot_test_flat_frac = ((frac_test_flat_def-frac_test_flat_def.iloc[0,:]))
np.savetxt('bot_test_cyl.txt',bot_test_cyl_frac)
np.savetxt('bot_test_junc.txt',bot_test_junc_frac)
np.savetxt('bot_test_flat.txt',bot_test_flat_frac)
np.savetxt('bot_test_cyl_def.txt',frac_test_cyl_def)
np.savetxt('bot_test_junc_def.txt',frac_test_junc_def)
np.savetxt('bot_test_flat_def.txt',frac_test_flat_def)

print (PE_fract)


np.savetxt('bot_PC-frac.txt', PC_fract)
np.savetxt('bot_PE-frac.txt', PE_fract)


minor_component_fraction = PC_fract+PE_fract
...
if dt > 0:
    newdims = [*PE_fract.shape]
    newdims[0] = int(np.floor(float(PE_fract.shape[0]) / dt))
    reduced_array = np.zeros(newdims)
    for i in range(0, newdims[0]):
        reduced_array[i, :] = PE_fract[i * dt : (i + 1) * dt, :].mean(axis=0)
    PE_fraction = reduced_array
print (PE_fraction)
np.savetxt('PE_fraction_bot.txt', PE_fraction)
np.savetxt('PE_fraction_mean_bot.txt', PE_fraction.mean(axis=0))
print (PE_fraction.mean(axis=0))
...
plt.title("PP-PC-fraction_bot")
plt.show()

df2 = pd.DataFrame(PE_fraction)
