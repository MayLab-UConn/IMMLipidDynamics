...
print (xy_inrange)
print (xy_means)
#np.savetxt ('xy_mean.txt',center_coords.xyz.squeeze()[xy_inrange, 0:2].mean(axis=0))  #COM both x and y dimesnions - dummy atoms in the cylinder inner and outer regions
...
print ('bot_cylinder region:' + str (unified_coordinate[in_cylinder].max()))
print ('bot_junction region:' + str (unified_coordinate[in_junction].max()))
print ('bot_flat region:' + str (unified_coordinate[in_flat].max()))
...
#np.savetxt ('sorted.txt', sorted_data)    # note that we don't bother getting the max value - when we digitize, anything right
...
print (format_bins)
np.savetxt('bin_label-bot.txt', format_bins, fmt='%s')
...
np.savetxt('bin_rho_bot.txt', bin_rho)
np.savetxt('bin_z_bot.txt', bin_z)
...
###########################################################################################
#bot_popc_cyl = ((total_popc.iloc[:, 0:11]-total_popc.iloc[0,:]).sum(axis=1))
#bot_popc_junc = (total_popc.iloc[:, 11:24]-total_popc.iloc[0,:]).sum(axis=1)
#bot_popc_flat = (total_popc.iloc[:, 24:]-total_popc.iloc[0,:]).sum(axis=1)
##########with respect to the total number of lipids in leaflet
total_popc_cyl_def = pd.DataFrame(((total_popc.iloc[:, 0:11].sum(axis=1))))
print ('checking new cylinder regions')
print (total_popc_cyl_def)
bot_popc_cyl = ((total_popc_cyl_def-total_popc_cyl_def.iloc[0,:]))
total_popc_junc_def = pd.DataFrame(((total_popc.iloc[:, 11:24].sum(axis=1))))
bot_popc_junc = ((total_popc_junc_def-total_popc_junc_def.iloc[0,:]))
total_popc_flat_def = pd.DataFrame(((total_popc.iloc[:, 24:].sum(axis=1))))
bot_popc_flat = ((total_popc_flat_def-total_popc_flat_def.iloc[0,:]))



####original with respect to the first frame
#bot_popc_cyl = (((total_popc.iloc[:, 0:11]-total_popc.iloc[0,:])/total_popc.iloc[0,:]).sum(axis=1))
#bot_popc_junc = (((total_popc.iloc[:, 11:24]-total_popc.iloc[0,:])/total_popc.iloc[0,:]).sum(axis=1))
#bot_popc_flat = (((total_popc.iloc[:, 24:]-total_popc.iloc[0,:])/total_popc.iloc[0,:]).sum(axis=1))
np.savetxt('bot_popc_count.txt',(10463-total_popc.sum(axis=1)))
np.savetxt('bot_popc_cyl.txt',bot_popc_cyl)
np.savetxt('bot_popc_junc.txt',bot_popc_junc)
np.savetxt('bot_popc_flat.txt',bot_popc_flat)
np.savetxt('bot_popc_cyl_def.txt',total_popc_cyl_def)
np.savetxt('bot_popc_junc_def.txt',total_popc_junc_def)
np.savetxt('bot_popc_flat_def.txt',total_popc_flat_def)
...
total_test =pd.DataFrame(test_bin/2526)
print (total_test.sum(axis=1))
np.savetxt('bot_test_count.txt',(2526-total_test.sum(axis=1)))
#####with respect to the total leaflet number
total_test_cyl_def = pd.DataFrame(((total_test.iloc[:, 0:11].sum(axis=1))))
print ('checking new cylinder regions-test')
print (total_test_cyl_def)
bot_test_cyl = ((total_test_cyl_def-total_test_cyl_def.iloc[0,:]))
total_test_junc_def = pd.DataFrame(((total_test.iloc[:, 11:24].sum(axis=1))))
bot_test_junc = ((total_test_junc_def-total_test_junc_def.iloc[0,:]))
total_test_flat_def = pd.DataFrame(((total_test.iloc[:, 24:].sum(axis=1))))
bot_test_flat = ((total_test_flat_def-total_test_flat_def.iloc[0,:]))

#############oroginal-with respect to the initial frame
#bot_test_cyl = (((total_test.iloc[:, 0:11]-total_test.iloc[0,:])/total_test.iloc[0,:]).sum(axis=1))
#bot_test_junc = (((total_test.iloc[:, 11:24]-total_test.iloc[0,:])/total_test.iloc[0,:]).sum(axis=1))
#bot_test_flat = (((total_test.iloc[:, 24:]-total_test.iloc[0,:])/total_test.iloc[0,:]).sum(axis=1))
np.savetxt('bot_test_cyl.txt',bot_test_cyl)
np.savetxt('bot_test_junc.txt',bot_test_junc)
np.savetxt('bot_test_flat.txt',bot_test_flat)
np.savetxt('bot_test_cyl_def.txt',total_test_cyl_def)
np.savetxt('bot_test_junc_def.txt',total_test_junc_def)
np.savetxt('bot_test_flat_def.txt',total_test_flat_def)
...
np.savetxt('bot_PC-frac.txt', PC_fract)
np.savetxt('bot_TS-frac.txt', TS_fract)
...
print (PC_fraction)
np.savetxt('PC_fraction_bot.txt', PC_fraction)
np.savetxt('PC_fraction_mean_bot.txt', PC_fraction.mean(axis=0))
...
print (TS_fraction)
np.savetxt('TS_fraction_bot.txt', TS_fraction)
np.savetxt('TS_fraction_mean_bot.txt', TS_fraction.mean(axis=0))
